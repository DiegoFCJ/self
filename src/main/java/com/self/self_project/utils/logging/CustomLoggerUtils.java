package com.self.self_project.utils.logging;
...
public class CustomLoggerUtils {
    private static final Logger logger = LoggerFactory.getLogger(CustomLoggerUtils.class);
    ...
    private static String componentCheck(String operationType) {
        ...
        if (operationType.contains("back")) {
            component = "BACK";
        } else if (operationType.contains("front")) {
            component = "FRONT";
        } else if (operationType.contains("driver")) {
            component = "DRIVER";
        } else if (operationType.contains("db")) {
            component = "DB";
        } else {
            component = "GENERAL";
        }
    ...
    private static void log(String level, String operationType, String description) {
        ...
        Logger componentLogger = LoggerFactory.getLogger("com.self.self_project." + component.toLowerCase());
    ...
    private static StackTraceElement findCallerStackTraceElement() {
        ...
        for (int i = 3; i < stackTrace.length; i++) {
            ...
            if (!className.contains("utils.logging.CustomLoggerUtils") && !className.startsWith("java.")) {
    ...
    private static String determineComponent() {
        ...
        String classUpperFolder = getClassNameAfter(caller.getClassName(), "self_project");
        String component;
    ...
    private static File createLogFile(String logDirectory) {
        return new File(logDirectory + "/self-manager_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss")) + ".log");
    }
    ...
    private static String formatLogMessage(String level, String operationType, String description) {
        ...
        return String.format(
            ...
            threadId,                                                  // Thread ID
            level,                                                     // Log level
            getClassNameAfter(caller.getClassName(), "self_project"), // Class name
    ...
    /**
     * Creates a ChromeDriverService with logging capabilities.
     *
     * @return An instance of ChromeDriverService.
     */
    public static ChromeDriverService createChromeDriverServiceWithLogging() {
        ChromeDriverService.Builder builder = new ChromeDriverService.Builder();
        String componentTocheck;
        componentTocheck = componentCheck("DRIVER");
        String logPath = String.format("LOGS/%s/BASE-APP/%s", todayDate, componentTocheck) + ".log";

        File logFile = new File(logPath);

        // Ensure the directory exists
        if (!logFile.getParentFile().exists() && !logFile.getParentFile().mkdirs()) {
            error("DRIVER", "Failed to create directory for ChromeDriver logs.");
        }
        ...
        try {
            service.start();
            info("DRIVER", "ChromeDriver service started at " + logFile.getAbsolutePath());
        } catch (Exception e) {
            error("DRIVER", "Failed to start ChromeDriver: " + e.getMessage());
        }
        return service;
    }
    ...
    public static void cleanEmptyLogFiles() {
        ...
        System.out.println("Log Directory Path: " + logDirectoryPath); // Debugging output
        ...
        if (logDirectory.exists() && logDirectory.isDirectory()) {
            ...
            if (componentDirs != null) {
                for (File componentDir : componentDirs) {
                    System.out.println("Checking component directory: " + componentDir.getAbsolutePath()); // Debugging output
                    ...
                    if (subDirs != null) {
                        for (File subDir : subDirs) {
                            System.out.println("Found subdirectory: " + subDir.getAbsolutePath()); // Debugging output
                            ...
                            if (logFiles != null) {
                                ...
                                for (File logFile : logFiles) {
                                    System.out.println("Found log file: " + logFile.getAbsolutePath()); // Debugging output
                                    ...
                                    if (logFile.length() == 0) {
                                        ...
                                        System.out.println("File is not empty: " + logFile.getAbsolutePath()); // Debugging output
                                    }
                                }
